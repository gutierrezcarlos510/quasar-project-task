<template>
  <q-page class="flex flex-center">
    <div class="q-pa-xl q-ma-xl q-gutter-sm full-width">
      <q-table title="Productos" :rows="dataProducts" :columns="columns" row-key="name" :filter="filter" binary-state-sort no-data-label="Sin datos registrados">
        <template v-slot:top>
          <div>
            <h4 class="q-py-none q-my-none">Productos Electrónicos</h4>
            <div class="row full-width">
              ...
              <div class="col-5">
                <q-input  dense debounce="200" v-model="size" @update:model-value="filtrarPorSize()" clearable>
                  <template v-slot:append>
                    <q-icon name="tag" />
                  </template>
                  <template v-slot:prepend>
                    <small>Filtrar mayor que</small>
                  </template>
                </q-input>
              </div>
              <div class="col-2"><q-btn flat outline dense color="primary" label="Adicionar" icon="add"  @click="mostrarModal" ></q-btn></div>
            </div>
          </div>
            ...
            <q-dialog v-model="confirm" persistent>
              <q-card>
                ...
                <q-card-section class="row items-center">
                  ...
                  <div class="col-6">{{ editedItem.precio }}</div>
                  <div class="col-6"><strong>Cantidad</strong></div>
                  <div class="col-6">{{ editedItem.cantidad }}</div>
                  <div class="col-6"><strong>Categoria</strong></div>
                  <div class="col-6">{{ editedItem.categoriaId.nombre }}</div>
                </q-card-section>
              ...
              <q-card>
                ...
                <q-card-section>
                  <div class="row q-col-gutter-md full-width q-pa-sm">
                    ...
                    <div class="col-6">
                      <q-input type="number" v-model.number="editedItem.precio" label="Precio">
                        <template v-slot:prepend>
                    ...
                    <div class="col-6">
                      <q-input type="number" v-model.number="editedItem.cantidad" label="Cantidad">
                        <template v-slot:prepend>
                          <q-icon name="tag" />
                        </template>
                      </q-input>
        ...
        <template v-slot:body="props">
          <q-tr :props="props">
            ...
            <q-td key="precio" :props="props">
              <div class="text-pre-wrap">{{ props.row.precio }}</div>
            </q-td>
            <q-td key="cantidad" :props="props">
              {{ props.row.cantidad }}
            </q-td>
            <q-td key="categoriaId" :props="props">
              {{ props.row.categoriaId.nombre }}
            </q-td>
...
const columns= ref([
  ...
  { name: "categoriaId", label: "Categoria", field: "categoriaId" },
  {
    name: "actions",
    label: "Acciones",
    field: "actions"
  ...
  const filtrarPorSize = async() => {
    loading.value = true;
    await productoStore.listActive(size.value).then(resp => {
      dataProducts.value = resp;
      loading.value = false;
    });
  }
const addRow = async() => {
  editedItem.value.categoriaId = categorias.value.find( cat => cat.id === editedItem.value.categoriaId);
  if (editedIndex.value > -1) {
    editedItem.value.id = parseInt(editedItem.value.id);
    await productoStore.update(editedItem.value);
    dataProducts.value = await productoStore.listActive(size.value);
  } else {
    let maxId = 0;
    ...
    const resp = await productoStore.save(editedItem.value);
    dataProducts.value = await productoStore.listActive(size.value);
    console.log(resp);
...
const deleteItem = async(item) => {
  await productoStore.remove(item.id);
  Notify.create({
    type: 'negative',
...
onMounted(async() => {
  loading.value = true;
  dataProducts.value = await productoStore.listActive(size.value);
  categorias.value = await categoriaStore.listActive();
  loading.value = false;
});
</script>
